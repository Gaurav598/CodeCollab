...
export function ExecutionPanel({ fileId, language, getCode, disabled }: ExecutionPanelProps) {
  ...
  return (
    <div className="border-t border-border bg-background flex flex-col min-h-[200px] h-64">
      <div className="flex items-center px-2 py-2 border-b border-border shrink-0 gap-2">
        {/* Left Half (Aligns with Program Input) */}
        <div className="flex-1 flex items-center justify-between">
          <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider pl-1">Execution Panel</div>
          <button
            type="button"
            onClick={() => { setResult(null); setError(null); setStdin(""); }}
            className="px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider rounded border border-border text-muted-foreground hover:bg-muted hover:text-primary shadow-sm hover:shadow-lg hover:-translate-y-1 active:translate-y-0 active:shadow-sm transition-all duration-300"
          >
            Clear
          </button>
        </div>
        
        {/* Right Half (Aligns with Program Output) */}
        <div className="flex-1 flex items-center justify-between">
          <button
            type="button"
            onClick={handleRun}
            disabled={disabled || running}
            className="px-4 py-1.5 text-xs font-bold uppercase tracking-wider rounded bg-green-600 hover:bg-green-500 disabled:opacity-50 disabled:cursor-not-allowed text-white shadow-sm hover:shadow-lg hover:-translate-y-1 active:translate-y-0 active:shadow-sm transition-all duration-300 flex items-center gap-2"
          >
            {running ? (
              <>
          ...
          {result && (
            <span className="text-xs font-medium text-muted-foreground bg-muted px-2 py-1 rounded border border-border mr-1">
      ...
      <div className="flex flex-1 min-h-0 gap-2 p-2 bg-background">
        {/* Input Section */}
